...
class SubmissionControllerTest {
    ...
    @Autowired
    private SQLDDLTaskRepository repository;

    @Autowired
    private SQLDDLTaskGroupRepository groupRepository;

    @Autowired
    private SQLDDLSubmissionRepository submissionRepository;
    ...
    void initDb() {
        ...
        var group = this.groupRepository.save(new SQLDDLTaskGroup(1L, TaskStatus.APPROVED, 1, 10));
        var task = this.repository.save(new SQLDDLTask(1L, BigDecimal.TWO, TaskStatus.APPROVED, group, 5));
        this.taskId = task.getId();

        var submission = new SQLDDLSubmission("test-user", "test-id", task, "de", 3, SubmissionMode.SUBMIT, "5");
        ...
        this.ungraded = this.submissionRepository.save(new SQLDDLSubmission("test-user", "test-id", task, "de", 3, SubmissionMode.SUBMIT, "5")).getId();
    }
    ...
    void submitInForegroundWithPersist() {
        given()
            ...
            .body(new SubmitSubmissionDto<>("test-user", "test-id", this.taskId, "de", SubmissionMode.SUBMIT, 3, new SQLDDLSubmissionDto("2")))
            // WHEN
            .when()
            .post("/api/submission")
            // THEN
    ...
    void submitInForegroundWithoutPersist() {
        given()
            ...
            .contentType(ContentType.JSON)
            .body(new SubmitSubmissionDto<>("test-user", "test-id", this.taskId, "de", SubmissionMode.SUBMIT, 3, new SQLDDLSubmissionDto("5")))
            // WHEN
            .when()
            .post("/api/submission")
            // THEN
    ...
    void submitShouldReturnBadRequest() {
        given()
            ...
            .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.SUBMIT_API_KEY)
            .contentType(ContentType.JSON)
            .body(new SubmitSubmissionDto<>("test-user", "test-id", this.taskId, "it", SubmissionMode.SUBMIT, 3, new SQLDDLSubmissionDto("5")))
            // WHEN
            .when()
            .post("/api/submission")
            // THEN
    ...
    void submitShouldReturnBadRequestOnInvalidTaskId() {
        given()
            ...
            .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.SUBMIT_API_KEY)
            .contentType(ContentType.JSON)
            .body(new SubmitSubmissionDto<>("test-user", "test-id", this.taskId + 1, "de", SubmissionMode.SUBMIT, 3, new SQLDDLSubmissionDto("5")))
            // WHEN
            .when()
            .post("/api/submission")
            // THEN
        ...
        var threads = IntStream.range(0, 100)
            .mapToObj(i -> CompletableFuture.supplyAsync(() -> {
                given()
                    ...
                    .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.SUBMIT_API_KEY)
                    .contentType(ContentType.JSON)
                    .body(new SubmitSubmissionDto<>("test-user-" + i, "test-id", this.taskId, "de", SubmissionMode.SUBMIT, 3, new SQLDDLSubmissionDto("5")))
                    // WHEN
                    .when()
                    .post("/api/submission")
                    // THEN
