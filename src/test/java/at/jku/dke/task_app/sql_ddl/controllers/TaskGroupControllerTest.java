...
class TaskGroupControllerTest {
    ...
    @Autowired
    private SQLDDLTaskGroupRepository repository;
    ...
    void initDb() {
        ...
        this.taskGroupId = this.repository.save(new SQLDDLTaskGroup(1L, TaskStatus.APPROVED, 1, 5)).getId();
    }
    ...
    void createShouldReturnCreated() {
        given()
            ...
            .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.CRUD_API_KEY)
            .contentType(ContentType.JSON)
            .body(new ModifyTaskGroupDto<>("sql-ddl", TaskStatus.APPROVED, new ModifySQLDDLTaskGroupDto(1, 5)))
            // WHEN
            .when()
    ...
    void createShouldReturnBadRequestOnInvalidBody() {
        given()
            ...
            .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.CRUD_API_KEY)
            .contentType(ContentType.JSON)
            .body(new ModifyTaskGroupDto<>("", TaskStatus.APPROVED, new ModifySQLDDLTaskGroupDto(1, 5)))
            // WHEN
            .when()
            .post("/api/taskGroup/{id}", this.taskGroupId + 2)
            // THEN
    ...
    void updateShouldReturnOk() {
        given()
            ...
            .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.CRUD_API_KEY)
            .contentType(ContentType.JSON)
            .body(new ModifyTaskGroupDto<>("sql-ddl", TaskStatus.APPROVED, new ModifySQLDDLTaskGroupDto(1, 10)))
            // WHEN
            .when()
    ...
    void updateShouldReturnBadRequestOnInvalidBody() {
        given()
            ...
            .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.CRUD_API_KEY)
            .contentType(ContentType.JSON)
            .body(new ModifyTaskGroupDto<>("sql", TaskStatus.APPROVED, new ModifySQLDDLTaskGroupDto(1, 10)))
            // WHEN
            .when()
            .put("/api/taskGroup/{id}", this.taskGroupId)
            // THEN
    ...
    void mapToDto() {
        // Arrange
        var taskGroup = new SQLDDLTaskGroup(1, 5);
