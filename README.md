# eTutor Task-App: SQL DDL

This application provides a REST-interface for following task type: sql_ddl.
...
This project **can** be used as a template for new task apps. Replace all occurrences of `SQLDDL`/`SQL DDL`/`sql-ddl`/`sql_ddl` with the name of your task
type.

Task-App repositories should start with `task-app-` and end with the task type (e.g. `task-app-sql-ddl`).
...
docker run -p 8090:8081 \ 
  -e SPRING_DATASOURCE_URL="jdbc:postgresql://postgres:5432/etutor_sql_ddl" \
  -e SPRING_DATASOURCE_USERNAME=etutor_sql_ddl \
  -e SPRING_DATASOURCE_PASSWORD=myPwd \
  -e SPRING_FLYWAY_USER=etutor_sql_ddl_admin \
  ...
  etutorplusplus/task-app-sql-ddl
```
...
services:
    task-app-sql-ddl:
        image: etutorplusplus/task-app-sql-ddl
        restart: unless-stopped
        ports:
            -   target: 8081
                published: 8090
        environment:
            SPRING_DATASOURCE_URL: jdbc:postgresql://postgres:5432/etutor_sql_ddl
            SPRING_DATASOURCE_USERNAME: etutor_sql_ddl
            SPRING_DATASOURCE_PASSWORD: myPwd
            SPRING_FLYWAY_USER: etutor_sql_ddl_admin
